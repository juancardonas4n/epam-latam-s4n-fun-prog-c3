...
def profArbolBin(ab:ArbolBinInt):Int = (ab: @unchecked) match {
  case Hoja(_)     => 0
  case BinInt(i,d) => 1 + scala.math.max(profArbolBin(i),
					 profArbolBin(d))
}

def sumaArbolBin(ab:ArbolBinInt):Int = (ab: @unchecked) match {
  case Hoja(v)     => v
